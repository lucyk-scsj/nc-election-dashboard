...
A public, auto-updating dashboard of actionable NC absentee/provisional data — built to answer "what's still fixable?" rather than just showing final counts. Pulls directly from the NC State Board of Elections public data files every day via GitHub Actions, and is hosted for free on GitHub Pages.
...
In repo settings → Actions → General → Workflow permissions, choose "Read and write permissions" (the workflow needs to commit the daily data update).
Push to main — the first workflow run will publish the site. After that it runs automatically every day (see the cron schedule in .github/workflows/update-data.yml); you can also trigger it manually from the Actions tab any time (e.g. the morning after Election Day).
